...
const getAccountsFromDevice = async ({ device: { deviceId }, network }) => {
  console.info("hwManager getAccountsFromDevice device", deviceId);
  const accounts = [];
  let account = {};
  for (let index = 0; index === accounts.length; index++) {
    ...
    // eslint-disable-next-line no-await-in-loop
    console.info("hwManager getAccountsFromDevice getAccount", { network, publicKey });
    account = await getAccount({ network, params: { publicKey } }, 'LSK');
    console.info("hwManager getAccountsFromDevice getAccount account", account);
 ...
 * This function is used for sign a transaction with HW.
 */
